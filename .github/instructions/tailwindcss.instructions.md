...
- Put theme variables in `tailwindcss/theme/**`, custom variants in `tailwindcss/variants/**`, and component class utilities in `tailwindcss/components/**`.
...
- Use `@apply` in every possible scenario where Tailwind can express the declaration, including spacing, sizing, typography, color, backgrounds, borders, radius, layout, focus rings, states, dark mode, arbitrary values, arbitrary properties, and CSS custom property assignment.
...
- Use raw CSS declarations only when Tailwind utilities are not expressive enough or would make the rule harder to understand, such as long pseudo-element setup that cannot compile through `@apply`, data URI backgrounds, `scrollbar-gutter`, complex calculated values, or browser-specific compatibility declarations.
...
- Remove dead utilities, obsolete variants, and duplicated declarations when refactoring.
